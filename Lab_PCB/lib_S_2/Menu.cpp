...
namespace Menu{
	...
	void menu(Prog3_S_2::PCB &p) {
		void (*fptr[])(Prog3_S_2::PCB&) = {nullptr, inputCon, printPCB, checkCon, connectCon, printConSingleType, distCon,
	...
	void printConSingleType(Prog3_S_2::PCB &p){
		...
		Prog3_S_2::PCB temp = p.selType(type);
		std::cout << temp;
	}
...
//		std::cout << p-- << std::endl;//проверка для постфиксного оператора
	...
	void anotherMenu(Prog3_S_2::PCB &p) {
		...
		void (*fptr[])(Prog3_S_2::PCB&) = {nullptr, inputCon, printPCB, checkCon, connectCon, printConSingleType, distCon,
										   printContact, invertPCB, deleteLast, anotherMenu};
		void (*fptr2[])(Prog3_S_2::PCB&, Prog3_S_2::PCB&) = {mergePCB, comparePCB, sumPCB};
